...
- 下一步若继续研究容量，`signal_fill_timing_audit` 与 `blocked_entry_event_export` 已完成；继续推进 `replay_consistency_audit -> stale_slot_continuation_review -> blocked_candidate_vs_stale_slot_review`。在 consistency audit 未通过前，不计算 replacement future outcome，不新增生产过滤器。
