...
| `entry_reclaim_min_atr_enabled=true` | 未部署 | `atr_reclaim_0_35` 阈值敏感性进入 `candidate_keep_review`，仍需交易级归因 | 决策：暂不部署 |
...
| `atr_reclaim` 阈值敏感性 | 0.10、0.15、0.25、0.35 哪个 reclaim margin 更稳 | `entry_reclaim_min_atr` | 2024-07-01 -> 2026-06-01 | 0.10/0.15 近端净收益和 PF 退化；0.35 两段净收益、PF、MDD 均改善 | `candidate_keep_review` for `atr_reclaim_0_35` | 尚未做交易级归因，不能部署 |
...
- ATR reclaim 门槛能提升平均交易质量；较低阈值 `0.10/0.15` 不稳，`0.35` 当前最强，但仍可能存在少数交易或路径驱动风险。
...
- ATR reclaim 的最佳阈值可能高于 0.25；更强 reclaim margin 可能过滤掉弱确认，但也需要确认不是少数赢家驱动。
...
- 不部署 `atr_reclaim_0_35`，先做交易级归因 keep review。
