...
- After creating a Git commit, push the current branch to `origin` before considering the task finished, unless the user explicitly asks not to push or the push fails due to network/auth/remote conflicts.
...
- If a change cannot be committed or pushed, record the reason in `dailylog.md` and tell the user clearly.
