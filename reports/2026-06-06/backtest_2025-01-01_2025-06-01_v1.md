---
created: 2026-06-06 18:30:57 CST
tags:
  - crypto
  - trading-system
  - backtest
backtest_run_id: 8e60ced56843
report_version: v1
---
...
- 最终权益：10,376.65 USDT
- 净收益：3.77%
- 代码 commit：`dd897e00657ea65258c5239a0570f42fa7b62e42`
...
| Open trades（仍开放持仓） | 2 |
| Win rate（胜率） | 66.67% |
| Profit factor（盈利因子） | 4.80 |
| Avg R（平均R倍数） | 1.43 |
| Net return（净收益率） | 3.77% |
| Max drawdown（最大回撤） | 357.77 / 3.34% |
| Intrabar max drawdown（K线内最大回撤） | 325.56 / 3.05% |
| TP1 touched rate（第一止盈触达率） | 66.67% |
| TP2 close rate（第二止盈平仓率） | 66.67% |
| Stop rate（止损率） | 33.33% |
| Fee drag（手续费拖累） | 6.13 USDT |
| Tail max single loss（最大单笔亏损） | -112.49 USDT |
| CAGR（年化复合收益率） | 9.35% |
| Sharpe（夏普比率） | 1.31 |
| Sortino（索提诺比率） | 0.95 |
| Exposure（持仓暴露时间） | 32.89% |
| Turnover（换手率） | 0.67 |
...
|---|---|---|---:|---:|---:|---:|---:|---:|---:|---|
| `BTCUSDT` | CLOSED（已按TP2平仓） | 2025-04-12T08:00:00+00:00 | 83,637.41 | 108,712.75 | 0.01 | 279.39 | 278.53 | 2.77 | 0.86 | TP2 hit; paper trade closed. |
| `ETHUSDT` | CLOSED（已按TP2平仓） | 2025-04-25T20:00:00+00:00 | 1,781.57 | 2,078.51 | 0.88 | 262.66 | 261.30 | 2.56 | 1.37 | TP2 hit; paper trade closed. |
| `BTCUSDT` | STOPPED（已止损） | 2025-05-24T12:00:00+00:00 | 109,590.95 | 104,421.38 | 0.02 | -109.35 | -112.49 | -1.05 | 3.14 | Stop loss hit. |

## 回测结束仍开放

| Symbol（交易对） | Status（状态） | Entry（入场价） | Qty（数量） | Unrealized Handling（未实现盈亏处理） | Notes（备注） |
|---|---|---:|---:|---|---|
| `SOLUSDT` | ENTERED（已入场） | 170.76 | 4.90 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
| `ETHUSDT` | ENTERED（已入场） | 2,481.78 | 0.44 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
