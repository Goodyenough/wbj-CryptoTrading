...
# 回测报告 2025-09-01 至 2026-06-01 v4

- 回测 ID：`b6a1661f9faa`
- 交易对：`0GUSDT`, `1000CHEEMSUSDT`, `1000SATSUSDT`, `2ZUSDT`, `AAVEUSDT`, `ACEUSDT`, `ADAUSDT`, `AEVOUSDT`, `AIUSDT`, `ALCXUSDT`, `ALICEUSDT`, `ALLOUSDT`, `ALPINEUSDT`, `ALTUSDT`, `APEUSDT`, `APTUSDT`, `ARBUSDT`, `ARKMUSDT`, `ARUSDT`, `ASTERUSDT`, `ATUSDT`, `AUCTIONUSDT`, `AVAXUSDT`, `AVNTUSDT`, `AXSUSDT`, `BANANAS31USDT`, `BARDUSDT`, `BCHUSDT`, `BELUSDT`, `BERAUSDT`, `BIOUSDT`, `BNBUSDT`, `BNSOLUSDT`, `BONKUSDT`, `BREVUSDT`, `BROCCOLI714USDT`, `BTCUSDT`, `CAKEUSDT`, `CFGUSDT`, `CFXUSDT`, `CHIPUSDT`, `COWUSDT`
- UTC 区间：2025-09-01T00:00:00+00:00 -> 2026-06-01T00:00:00+00:00
- 初始权益：10,000.00 USDT
- 最终权益：9,176.84 USDT
- 净收益：-8.23%
- 代码 commit：`ebad0d7272e31f5f5e7dc71d4ced822d98e29808`
- 样本是否充分：false
...
- Selected symbols per refresh / 每次入选数量：min=1, avg=3.80, max=12
- Top selected symbols / 最常入选：`BTCUSDT`(274), `BNBUSDT`(256), `ADAUSDT`(115), `ASTERUSDT`(80), `AVAXUSDT`(78), `BCHUSDT`(35), `AVNTUSDT`(24), `CHIPUSDT`(18), `CAKEUSDT`(15), `ARBUSDT`(14)
- Filter counts / 过滤统计：
```json
{
  "missing_1h": 1171,
...
| Open trades（仍开放持仓） | 2 |
| Win rate（胜率） | 15.79% |
| Profit factor（盈利因子） | 0.48 |
| Avg R（平均R倍数） | -0.45 |
| Net return（净收益率） | -8.23% |
| Max drawdown（最大回撤） | 1,695.33 / 15.99% |
| Intrabar max drawdown（K线内最大回撤） | 1,686.85 / 15.97% |
| TP1 touched rate（第一止盈触达率） | 31.58% |
| TP2 close rate（第二止盈平仓率） | 15.79% |
| Stop rate（止损率） | 84.21% |
| Fee drag（手续费拖累） | 37.06 USDT |
| Tail max single loss（最大单笔亏损） | -108.51 USDT |
| CAGR（年化复合收益率） | -10.85% |
| Sharpe（夏普比率） | -0.89 |
| Sortino（索提诺比率） | -0.79 |
...
| Cash（现金不交易） | 0.00% |
| Equal-weight symbols（等权持有本次币种） | -59.18% |
...
| `ADAUSDT` | STOPPED（已止损） | 2025-09-17T08:00:00+00:00 | 0.89 | 0.84 | 2,233.02 | -104.09 | -106.76 | -1.04 | 2.66 | Stop loss hit. |
| `ARBUSDT` | STOPPED（已止损） | 2025-09-18T00:00:00+00:00 | 0.52 | 0.48 | 2,519.39 | -106.79 | -108.51 | -1.03 | 1.72 | Stop loss hit. |
| `BONKUSDT` | STOPPED（已止损） | 2025-09-18T08:00:00+00:00 | 0.00 | 0.00 | 39,714,434.92 | -106.90 | -108.17 | -1.02 | 1.27 | Stop loss hit. |
| `BIOUSDT` | STOPPED（已止损） | 2025-09-21T00:00:00+00:00 | 0.18 | 0.16 | 4,650.71 | -105.99 | -107.07 | -1.02 | 1.09 | Stop loss hit. |
| `BNBUSDT` | STOPPED（已止损） | 2025-09-21T12:00:00+00:00 | 1,025.08 | 958.33 | 1.54 | -102.65 | -104.76 | -1.04 | 2.10 | Stop loss hit. |
| `CAKEUSDT` | STOPPED（已止损） | 2025-09-21T12:00:00+00:00 | 2.89 | 2.59 | 347.62 | -103.31 | -104.61 | -1.02 | 1.30 | Stop loss hit. |
| `ADAUSDT` | STOPPED（已止损） | 2025-10-02T00:00:00+00:00 | 0.85 | 0.77 | 1,277.57 | -101.59 | -103.00 | -1.02 | 1.41 | Stop loss hit. |
| `AVAXUSDT` | STOPPED（已止损） | 2025-10-02T20:00:00+00:00 | 30.78 | 28.35 | 41.99 | -102.17 | -103.88 | -1.03 | 1.71 | Stop loss hit. |
| `BNBUSDT` | STOPPED（已止损） | 2025-10-05T12:00:00+00:00 | 1,149.84 | 1,024.45 | 0.81 | -101.14 | -102.34 | -1.02 | 1.20 | Stop loss hit. |
| `APTUSDT` | STOPPED（已止损） | 2025-10-05T20:00:00+00:00 | 5.38 | 4.99 | 264.61 | -103.32 | -105.21 | -1.03 | 1.89 | Stop loss hit. |
| `BTCUSDT` | STOPPED（已止损） | 2025-10-21T16:00:00+00:00 | 113,819.76 | 104,407.30 | 0.01 | -95.43 | -96.95 | -1.03 | 1.52 | Stop loss hit. |
| `BTCUSDT` | STOPPED（已止损） | 2025-11-30T04:00:00+00:00 | 91,152.21 | 88,714.33 | 0.04 | -98.24 | -103.28 | -1.09 | 5.04 | Stop loss hit. |
| `BTCUSDT` | STOPPED（已止损） | 2025-12-03T08:00:00+00:00 | 93,325.63 | 82,482.85 | 0.01 | -92.90 | -93.93 | -1.02 | 1.03 | Stop loss hit. |
| `BNBUSDT` | STOPPED（已止损） | 2026-01-18T00:00:00+00:00 | 944.78 | 908.92 | 2.67 | -95.64 | -99.07 | -1.06 | 3.43 | Stop loss hit. |
| `BNBUSDT` | CLOSED（已按TP2平仓） | 2026-04-16T00:00:00+00:00 | 618.86 | 718.19 | 2.53 | 251.12 | 249.77 | 2.73 | 1.35 | TP2 hit; paper trade closed. |
| `ADAUSDT` | STOPPED（已止损） | 2026-04-17T00:00:00+00:00 | 0.25 | 0.23 | 4,587.39 | -92.46 | -94.00 | -1.03 | 1.54 | Stop loss hit. |
...
| `AVNTUSDT` | TP1_HIT（第一止盈已触达） | 0.14 | 5,550.21 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
...
```json
{
  ...
  "dynamic_universe_summary": {
    ...
    "created_at_utc": "2026-06-08T18:18:57+00:00",
    ...
    "selected_count_avg": 3.802919708029197,
