---
created: 2026-06-09 12:38:50 CST
tags:
  - crypto
  - trading-system
  - backtest
backtest_run_id: b8929f6a4f08
report_version: v3
...
# 回测报告 2025-01-01 至 2025-06-01 v3
...
|---|---:|
| Trades（计划总数） | 36 |
| Closed trades（已结束交易） | 14 |
| Open trades（仍开放持仓） | 2 |
| Win rate（胜率） | 7.14% |
| Profit factor（盈利因子） | 0.20 |
| Avg R（平均R倍数） | -0.75 |
| Net return（净收益率） | -10.37% |
| Max drawdown（最大回撤） | 1,297.89 / 12.67% |
| Intrabar max drawdown（K线内最大回撤） | 1,272.26 / 12.50% |
| TP1 touched rate（第一止盈触达率） | 14.29% |
| TP2 close rate（第二止盈平仓率） | 7.14% |
| Stop rate（止损率） | 92.86% |
| Fee drag（手续费拖累） | 20.39 USDT |
...
|---|---|---:|---:|---|---|
| `BNBUSDT` | ENTERED（已入场） | 652.04 | 3.49 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
| `AAVEUSDT` | ENTERED（已入场） | 255.84 | 2.27 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
...
```json
{
  ...
  "dynamic_universe_summary": {
    ...
    "created_at_utc": "2026-06-09T04:38:50+00:00",
