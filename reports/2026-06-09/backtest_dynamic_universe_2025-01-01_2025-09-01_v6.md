...
# 回测报告 2025-01-01 至 2025-09-01 v6

- 回测 ID：`713c3fd7cd6a`
- 交易对：`1000CATUSDT`, `1000CHEEMSUSDT`, `1000SATSUSDT`, `1INCHUSDT`, `AAVEUSDT`, `ACHUSDT`, `ACTUSDT`, `ACXUSDT`, `ADAUSDT`, `AGLDUSDT`, `AIUSDT`, `AIXBTUSDT`, `ALGOUSDT`, `ALPINEUSDT`, `ALTUSDT`, `AMPUSDT`, `ANIMEUSDT`, `API3USDT`, `APTUSDT`, `ARBUSDT`, `ARKMUSDT`, `ARKUSDT`, `ARPAUSDT`, `ARUSDT`, `ASRUSDT`, `ATOMUSDT`, `AUCTIONUSDT`, `AUSDT`, `AVAXUSDT`, `AXLUSDT`, `BABYUSDT`, `BANANAS31USDT`, `BCHUSDT`, `BERAUSDT`, `BIOUSDT`, `BMTUSDT`, `BNBUSDT`, `BNSOLUSDT`, `BOMEUSDT`, `BONKUSDT`, `BTCUSDT`, `CAKEUSDT`, `CETUSUSDT`, `CFXUSDT`, `CGPTUSDT`, `CKBUSDT`, `COMPUSDT`, `COOKIEUSDT`, `COWUSDT`
...
- Selected symbols per refresh / 每次入选数量：min=1, avg=5.93, max=18
- Top selected symbols / 最常入选：`BTCUSDT`(244), `BNBUSDT`(240), `ADAUSDT`(222), `AVAXUSDT`(141), `AAVEUSDT`(104), `BONKUSDT`(80), `ARBUSDT`(62), `APTUSDT`(53), `ACTUSDT`(32), `BERAUSDT`(27)
- Filter counts / 过滤统计：
```json
{
  "missing_1h": 4025,
...
|---|---:|
| Trades（计划总数） | 60 |
| Closed trades（已结束交易） | 30 |
| Open trades（仍开放持仓） | 3 |
| Win rate（胜率） | 20.00% |
| Profit factor（盈利因子） | 0.65 |
| Avg R（平均R倍数） | -0.26 |
| Net return（净收益率） | -8.13% |
| Max drawdown（最大回撤） | 1,498.15 / 14.69% |
| Intrabar max drawdown（K线内最大回撤） | 1,462.25 / 14.42% |
| TP1 touched rate（第一止盈触达率） | 30.00% |
| TP2 close rate（第二止盈平仓率） | 20.00% |
| Stop rate（止损率） | 80.00% |
| Fee drag（手续费拖累） | 38.97 USDT |
...
| Cash（现金不交易） | 0.00% |
| Equal-weight symbols（等权持有本次币种） | -39.86% |
...
| `BNBUSDT` | CLOSED（已按TP2平仓） | 2025-05-11T04:00:00+00:00 | 649.68 | 784.82 | 1.87 | 252.93 | 251.85 | 2.66 | 1.07 | TP2 hit; paper trade closed. |
...
| `AAVEUSDT` | STOPPED（已止损） | 2025-05-22T12:00:00+00:00 | 255.84 | 214.03 | 2.28 | -95.28 | -96.00 | -1.01 | 0.72 | Stop loss hit. |
| `BTCUSDT` | STOPPED（已止损） | 2025-06-06T16:00:00+00:00 | 105,460.26 | 98,767.81 | 0.01 | -93.44 | -95.41 | -1.04 | 1.97 | Stop loss hit. |
| `AVAXUSDT` | STOPPED（已止损） | 2025-06-10T00:00:00+00:00 | 21.79 | 19.25 | 37.15 | -94.27 | -95.31 | -1.02 | 1.04 | Stop loss hit. |
| `ADAUSDT` | STOPPED（已止损） | 2025-06-10T00:00:00+00:00 | 0.71 | 0.64 | 1,397.52 | -95.52 | -96.81 | -1.02 | 1.29 | Stop loss hit. |
| `ARBUSDT` | CLOSED（已按TP2平仓） | 2025-06-30T16:00:00+00:00 | 0.34 | 0.45 | 1,978.06 | 224.70 | 224.07 | 2.53 | 0.63 | TP2 hit; paper trade closed. |
| `ADAUSDT` | CLOSED（已按TP2平仓） | 2025-07-03T08:00:00+00:00 | 0.60 | 0.80 | 1,220.20 | 244.56 | 243.88 | 2.74 | 0.68 | TP2 hit; paper trade closed. |
| `AAVEUSDT` | STOPPED（已止损） | 2025-07-10T04:00:00+00:00 | 296.13 | 273.05 | 3.98 | -91.88 | -93.44 | -1.03 | 1.56 | Stop loss hit. |
| `APTUSDT` | STOPPED（已止损） | 2025-07-17T00:00:00+00:00 | 5.16 | 4.71 | 213.81 | -96.45 | -97.90 | -1.03 | 1.45 | Stop loss hit. |
| `ARBUSDT` | STOPPED（已止损） | 2025-07-17T04:00:00+00:00 | 0.43 | 0.38 | 2,047.84 | -97.57 | -98.71 | -1.02 | 1.14 | Stop loss hit. |
| `ADAUSDT` | STOPPED（已止损） | 2025-07-21T20:00:00+00:00 | 0.87 | 0.78 | 1,086.36 | -99.07 | -100.30 | -1.02 | 1.23 | Stop loss hit. |
| `BNBUSDT` | CLOSED（已按TP2平仓） | 2025-07-23T00:00:00+00:00 | 769.46 | 896.33 | 1.94 | 246.65 | 245.35 | 2.58 | 1.30 | TP2 hit; paper trade closed. |
| `CFXUSDT` | CLOSED（已按TP2平仓） | 2025-07-24T08:00:00+00:00 | 0.18 | 0.26 | 3,936.56 | 315.62 | 314.93 | 3.34 | 0.69 | TP2 hit; paper trade closed. |
| `CAKEUSDT` | STOPPED（已止损） | 2025-07-28T16:00:00+00:00 | 3.00 | 2.57 | 231.38 | -99.73 | -100.60 | -1.01 | 0.87 | Stop loss hit. |
| `CFXUSDT` | STOPPED（已止损） | 2025-07-30T00:00:00+00:00 | 0.21 | 0.18 | 3,304.96 | -96.53 | -97.39 | -1.02 | 0.86 | Stop loss hit. |
| `BCHUSDT` | STOPPED（已止损） | 2025-08-08T00:00:00+00:00 | 576.71 | 532.94 | 2.21 | -96.82 | -98.51 | -1.03 | 1.69 | Stop loss hit. |
| `APTUSDT` | STOPPED（已止损） | 2025-08-14T08:00:00+00:00 | 4.95 | 4.41 | 181.34 | -96.88 | -98.04 | -1.02 | 1.16 | Stop loss hit. |
| `ADAUSDT` | STOPPED（已止损） | 2025-08-23T12:00:00+00:00 | 0.91 | 0.81 | 937.41 | -97.24 | -98.34 | -1.02 | 1.10 | Stop loss hit. |
...
| `AVAXUSDT` | TP1_HIT（第一止盈已触达） | 23.09 | 48.46 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
| `BNBUSDT` | ENTERED（已入场） | 871.49 | 1.49 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
...
```json
{
  ...
  "dynamic_universe_summary": {
    ...
    "created_at_utc": "2026-06-08T17:20:53+00:00",
    ...
    "selected_count_avg": 5.930327868852459,
