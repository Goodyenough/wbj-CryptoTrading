...
- 最终权益：9,197.13 USDT
- 净收益：-8.03%
- 代码 commit：`4122fac9940471f04b25d0e8b37df709951a1732`
- 样本是否充分：false
...
| Avg R（平均R倍数） | -0.42 |
| Net return（净收益率） | -8.03% |
| Max drawdown（最大回撤） | 1,186.14 / 11.46% |
| Intrabar max drawdown（K线内最大回撤） | 1,173.15 / 11.43% |
| TP1 touched rate（第一止盈触达率） | 15.38% |
| TP2 close rate（第二止盈平仓率） | 15.38% |
| Stop rate（止损率） | 84.62% |
| Fee drag（手续费拖累） | 19.24 USDT |
| Tail max single loss（最大单笔亏损） | -105.37 USDT |
| CAGR（年化复合收益率） | -18.32% |
| Sharpe（夏普比率） | -1.49 |
| Sortino（索提诺比率） | -1.32 |
| Exposure（持仓暴露时间） | 57.17% |
| Turnover（换手率） | 2.05 |
...
|---|---|---:|---:|---|---|
| `SOLUSDT` | ENTERED（已入场） | 170.76 | 4.58 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
| `BNBUSDT` | ENTERED（已入场） | 668.24 | 3.71 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
| `TRXUSDT` | ENTERED（已入场） | 0.27 | 6,837.75 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
| `ETHUSDT` | ENTERED（已入场） | 2,683.68 | 0.44 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
...
```json
{
  ...
  "dynamic_universe_summary": {
    ...
    "created_at_utc": "2026-06-09T13:25:21+00:00",
