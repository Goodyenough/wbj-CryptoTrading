...
- 最终权益：8,864.10 USDT
- 净收益：-11.36%
- 代码 commit：`a26c80d053ce5a29474a2b4d61a270a441028b03`
- 样本是否充分：false
...
| Open trades（仍开放持仓） | 1 |
| Win rate（胜率） | 11.11% |
| Profit factor（盈利因子） | 0.31 |
| Avg R（平均R倍数） | -0.62 |
| Net return（净收益率） | -11.36% |
| Max drawdown（最大回撤） | 1,382.21 / 13.50% |
| Intrabar max drawdown（K线内最大回撤） | 1,296.71 / 12.79% |
| TP1 touched rate（第一止盈触达率） | 11.11% |
| TP2 close rate（第二止盈平仓率） | 11.11% |
| Stop rate（止损率） | 88.89% |
| Fee drag（手续费拖累） | 25.42 USDT |
| Tail max single loss（最大单笔亏损） | -104.33 USDT |
| CAGR（年化复合收益率） | -25.28% |
| Sharpe（夏普比率） | -2.14 |
| Sortino（索提诺比率） | -2.04 |
| Exposure（持仓暴露时间） | 74.17% |
| Turnover（换手率） | 2.06 |
...
|---|---|---:|---:|---|---|
| `UNIUSDT` | ENTERED（已入场） | 6.40 | 118.46 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
...
```json
{
  ...
  "dynamic_universe_summary": {
    ...
    "created_at_utc": "2026-06-09T18:34:22+00:00",
