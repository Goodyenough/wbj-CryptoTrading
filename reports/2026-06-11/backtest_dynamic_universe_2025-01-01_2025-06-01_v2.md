...
|---|---:|
| Trades（计划总数） | 92 |
| Closed trades（已结束交易） | 15 |
| Open trades（仍开放持仓） | 2 |
| Win rate（胜率） | 20.00% |
| Profit factor（盈利因子） | 0.53 |
| Avg R（平均R倍数） | -0.36 |
| Net return（净收益率） | -6.50% |
| Max drawdown（最大回撤） | 1,031.17 / 10.02% |
| Intrabar max drawdown（K线内最大回撤） | 969.53 / 9.52% |
| TP1 touched rate（第一止盈触达率） | 20.00% |
| TP2 close rate（第二止盈平仓率） | 13.33% |
| Stop rate（止损率） | 86.67% |
| Fee drag（手续费拖累） | 21.64 USDT |
| Tail max single loss（最大单笔亏损） | -105.45 USDT |
| CAGR（年化复合收益率） | -14.98% |
| Sharpe（夏普比率） | -1.08 |
| Sortino（索提诺比率） | -0.92 |
| Exposure（持仓暴露时间） | 56.84% |
| Turnover（换手率） | 2.01 |
...
| `PEPEUSDT` | STOPPED（已止损） | 2025-05-18T08:00:00+00:00 | 0.00 | 0.00 | 82,431,038.07 | 109.80 | 108.20 | 1.13 | 1.60 | Stop loss hit. |

## 回测结束仍开放

| Symbol（交易对） | Status（状态） | Entry（入场价） | Qty（数量） | Unrealized Handling（未实现盈亏处理） | Notes（备注） |
|---|---|---:|---:|---|---|
| `UNIUSDT` | ENTERED（已入场） | 6.40 | 123.84 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
| `BNBUSDT` | ENTERED（已入场） | 668.24 | 3.74 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
...
```json
{
  ...
  "dynamic_universe_summary": {
    ...
    "created_at_utc": "2026-06-11T07:48:30+00:00",
