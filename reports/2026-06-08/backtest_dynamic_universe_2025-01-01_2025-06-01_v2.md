...
| Open trades（仍开放持仓） | 1 |
| Win rate（胜率） | 9.09% |
| Profit factor（盈利因子） | 0.24 |
| Avg R（平均R倍数） | -0.70 |
| Net return（净收益率） | -7.77% |
| Max drawdown（最大回撤） | 925.94 / 9.20% |
| Intrabar max drawdown（K线内最大回撤） | 952.43 / 9.50% |
| TP1 touched rate（第一止盈触达率） | 9.09% |
| TP2 close rate（第二止盈平仓率） | 9.09% |
| Stop rate（止损率） | 90.91% |
| Fee drag（手续费拖累） | 13.06 USDT |
| Tail max single loss（最大单笔亏损） | -103.88 USDT |
| CAGR（年化复合收益率） | -17.75% |
| Sharpe（夏普比率） | -1.75 |
| Sortino（索提诺比率） | -0.86 |
| Exposure（持仓暴露时间） | 16.56% |
| Turnover（换手率） | 1.09 |
...
| `ADAUSDT` | STOPPED（已止损） | 2025-05-11T12:00:00+00:00 | 0.80 | 0.70 | 1,012.08 | -95.98 | -97.02 | -1.02 | 1.04 | Stop loss hit. |
| `AVAXUSDT` | STOPPED（已止损） | 2025-05-11T16:00:00+00:00 | 24.14 | 20.82 | 28.88 | -95.89 | -96.77 | -1.02 | 0.88 | Stop loss hit. |
| `APTUSDT` | STOPPED（已止损） | 2025-05-11T16:00:00+00:00 | 5.83 | 5.08 | 129.24 | -96.00 | -96.96 | -1.02 | 0.96 | Stop loss hit. |
| `AAVEUSDT` | CLOSED（已按TP2平仓） | 2025-05-12T16:00:00+00:00 | 219.33 | 262.60 | 5.67 | 245.40 | 244.31 | 2.55 | 1.09 | TP2 hit; paper trade closed. |
| `ARBUSDT` | STOPPED（已止损） | 2025-05-14T04:00:00+00:00 | 0.45 | 0.40 | 2,043.87 | -97.95 | -99.13 | -1.02 | 1.18 | Stop loss hit. |
| `ARBUSDT` | STOPPED（已止损） | 2025-05-23T08:00:00+00:00 | 0.42 | 0.37 | 1,876.02 | -96.16 | -97.18 | -1.02 | 1.02 | Stop loss hit. |

## 回测结束仍开放

| Symbol（交易对） | Status（状态） | Entry（入场价） | Qty（数量） | Unrealized Handling（未实现盈亏处理） | Notes（备注） |
|---|---|---:|---:|---|---|
| `AAVEUSDT` | ENTERED（已入场） | 255.84 | 2.31 | 按最后 close 计入净值，不计入胜率/profit_factor/avg_R | Open at backtest end; mark-to-market only. |
...
| WARNING | `ACTUSDT` | 4h | Large wick/range candle. |
| WARNING | `ACTUSDT` | 4h | Large wick/range candle. |
| INFO | n/a | n/a | Additional issues omitted: 378. |
...
```json
{
  ...
  "dynamic_universe_summary": {
    ...
    "created_at_utc": "2026-06-08T15:52:58+00:00",
