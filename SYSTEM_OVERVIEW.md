...
| ATR reclaim 门槛 | 默认关闭：`entry_reclaim_min_atr_enabled=false`；`atr_reclaim_0_35` 已进入 `candidate_keep_review_but_path_dependent`，改善主要来自 variant-only 新增赢家，尚未部署 |
...
4. 继续研究但尚未部署：`max_holding_bars_without_tp1=42`、`relative_strength_soft_gate`、`entry_reclaim_min_atr=0.35`；其中 `atr_reclaim_0_35` 已完成交易级归因，仍需人工复盘新增赢家与错过赢家的路径质量。
