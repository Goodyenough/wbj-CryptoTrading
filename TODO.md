...
- [ ] 设计 42 根条件式退出 A/B：满 42 根且未触 TP1 后，仅在 4h 收盘低于 EMA20 或入场价时退出，并与固定 42 根 `TIME_EXIT` 比较机会成本和 MDD。
...
- [ ] 从首次新版 `daily_full` 成功运行开始连续观察 5 天，并运行 `python main.py db stability --days 5`；当前进度 `2/5`（2026-06-13、2026-06-14 均成功：每天 1 scan、4 snapshots、三类报告齐全，无锁/重复/外键错误），继续等待 2026-06-15 至 2026-06-17 的自然日样本。
- [x] 准备并验收 `scripts/paper_4h_update.bat` 与 `scripts/install_4h_paper_task.ps1`：仅运行 `paper cycle`，固定 00:10、04:10、08:10、12:10、16:10，禁止 scan/add-from-scan，并设置 30 分钟上限与 `IgnoreNew` 防重入；安装脚本先以普通权限预检 5 天门槛，再要求管理员权限；临时库集成测试确认 cycle 不新增 scan/plan，只更新已有计划并写 run/event/snapshot/report/dashboard。
