...
- [ ] ?? prospective shadow observation schema??? reference baseline / `atr_reclaim_0_35_shadow` / research incumbent / challenger ???????capacity state?direct filtering ? path contribution?
- [ ] ???????????????? A/B?? prospective shadow observation ????????
- [ ] ?? prospective shadow observation MVP?????? `atr_reclaim_0_35` ? baseline/variant ?????????????????????????????? mature outcome review?
