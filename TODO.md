...
- [ ] 将 `max_holding_42x4h_no_tp1` 与当前 sensitive 组合叠加，跑两段非重叠 walk-forward（`2024-07-01 -> 2025-06-01` 和 `2025-06-01 -> 2026-06-01`），验证在 sensitive 默认配置基础上持仓时间过滤是否仍改善净收益和 MDD。需在 `experiments.toml` 新增组合实验定义，同时在 `abtest.py` 注册新 dimension（需包含 sensitive 六个参数 + `max_holding_bars_without_tp1`）。
...
- [x] 完成 SQLite 三周观察基础设施与主数据层切换：`runs`、`market_scans`、`paper_plans`、`paper_events`、`paper_snapshots`、WAL、30 秒 timeout、UTC 时间、daily_full run_id、snapshot、db-summary/events/export；`db-summary` 已直接汇总三周核心指标与 daily/4h 北京时间覆盖日期；paper update/report/dashboard 已读取结构化主表，legacy 表仅保留兼容镜像；故障注入测试证明单 plan 的状态、事件和快照原子回滚，且不阻断后续 plan。
...
- [x] 准备并验收 `scripts/paper_4h_update.bat` 与 `scripts/install_4h_paper_task.ps1`：仅运行 `paper cycle`，固定 00:10、04:10、08:10、12:10、16:10，禁止 scan/add-from-scan，并设置 30 分钟上限与 `IgnoreNew` 防重入；临时库集成测试确认 cycle 不新增 scan/plan，只更新已有计划并写 run/event/snapshot/report/dashboard。
