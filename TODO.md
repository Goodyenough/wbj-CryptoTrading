...
- [x] Build Dynamic Universe Backtest MVP: rebuild the universe daily from closed historical klines before selecting candidates. Source: universe snapshot smoke test on 2026-06-06.
- [ ] Run dynamic-universe A/B experiments without `--source-limit` after the kline cache is warm enough for a larger master list.
- [ ] Add negative/no-history kline cache handling so newly listed symbols without requested-period history are not re-fetched repeatedly during dynamic-universe smoke tests.
- [ ] Research a historical/delisted Binance symbol master list to reduce delisted-symbol survivorship bias in dynamic universe backtests.
