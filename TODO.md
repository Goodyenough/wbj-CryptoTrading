...
- [ ] **2026-08-16 至 2026-08-22：执行新 validation policy 的 7 天 observation epoch**。本阶段不修改 `config/settings.toml`，不启动新的 ATR、ranking、capacity replacement 或 TP1 策略实验。
  ...
  - [x] 只读审计 `ONDOUSDT` 计划 `9734a33dea2e`：统计 `RECLAIM_PENDING` 与 `API_DELAY_SKIPPED`，核对价格/`entry_high`/stop、`watch_expiry_bars` 适用性、重复事件和状态迁移；当前未证明存在 lifecycle bug，不得手动关闭计划或制造 terminal outcome。
  - [x] 已准备 TP1 部分止盈 50% 实验卡片：见 `reports/2026-08-16/tp1_partial_take_profit_50_experiment_card_2026-08-16_v1.md`；已写明唯一研究问题、baseline/variant、固定 symbols/日期窗口/4h 执行口径/手续费/滑点/容量、支持/否定/证据不足标准，以及实现前所需状态机和测试；未经批准不得写入生产配置或代码。
