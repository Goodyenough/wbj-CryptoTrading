...
### 01:32:42 +08:00 - 完成 TP1 部分止盈 50% 实验卡片
- 类型：研究文档 / TODO / 开发计划 / Git
...
- Git：`f76bff6` - `docs: close Claude research direction discussion`。
...
- Git：`dbccc22` - `docs: start Claude discussion on research direction`。
...
- Git：代码实现 commit `8f400c8`；日志引用通过后续文档提交补齐。
...
### 00:54:06 +08:00 - 复核 Claude §14 的 CMC 映射根因结论
- 类型：文档 / 研究讨论 / 数据库只读诊断 / Git
- 改动：阅读 Claude §14；只读重算 305 条有质量状态候选的 provider warning 分类，并核对 `data_validation.py` 的 CMC symbol 查询、`cmc_rank` 选择和 `cmc_id_overrides` 路径；在 `0814-与GPT讨论.md` 末尾追加 §15。
...
- ??????? `abandon_2023_2024_window_for_atr_reclaim_validation`??? `atr_reclaim_0_35_status` ??? `experimental_candidate_unvalidated`?
- ????? N0-N4 ??? diagnostic evidence???? 2023-2024 historical universe???? corrected N1??? path fork audit???? `config/settings.toml`?
- ?????? candidate recent-window eligibility audit????? prospective shadow observation schema?
- Git?????

### 12:11:54 +08:00 - ?? Stage N4 ?????? GPT ???
- ????? / GPT ?? / Git
- ????? `.gpt-review/current_packet.md`??? `.gpt-review/history/20260730-121135_packet.md`?? N0-N4 ???????? validation blocked ???N5 official-source mapping pipeline ?????????????? GPT ????
- ???? GPT ?????????? official-source mapping pipeline?????? impact triage?gate thresholds ?????????????????
- ????????????????????? N1????????????? `config/settings.toml`?
- ?????? `powershell -ExecutionPolicy Bypass -File C:\Users\10537\.codex\skills\gpt-plan-review\scripts\open-gpt-review.ps1 -PacketPath .gpt-review\current_packet.md`?????? ChatGPT ???????
- Git?????

### 11:44:46 +08:00 - ?? atr_reclaim Stage N4 historical master MVP
- ????? / ?? / ???? / TODO / ?? / Git
- ????? `scripts/n4_historical_master_mvp.py`??? N3 dataset ?? historical master MVP?blocking review queue ? raw summary?
- ???historical master rows=`413`?`active_current_master=266`?`excluded_by_strategy_universe_rule=20`?`historical_standard_gap_requires_mapping=127`?verdict=`historical_master_mvp_built_validation_blocked`?
- ???? MVP ????????????? A/B????????????????? 127 ? blocking review queue ? source-backed mapping ??????????
- ?????? `python -m compileall scripts\n4_historical_master_mvp.py`?`python tests\test_universe.py`?`python scripts\n4_historical_master_mvp.py ...`?
- Git?????

### 11:36:00 +08:00 - ?? atr_reclaim Stage N3 historical membership dataset MVP
- ????? / ?? / ???? / TODO / ?? / Git
- ????? `scripts/n3_historical_membership_dataset.py`??? N2 historical/current JSON???? historical symbols ??? leveraged token?stable/fiat/excluded base?nonstandard asset?rename/migration candidate ? standard spot gap?
- ???`missing_from_current_master=147`??? `excludable_missing_count=20`?`standard_gap_count=127`?`standard_gap_ratio_pct=32.32%`?verdict=`third_window_not_recoverable_without_historical_master`?
- ???????????????????????? corrected N1??????????????? historical master ?????
- ?????? `python -m compileall scripts\n3_historical_membership_dataset.py`?`python tests\test_universe.py`?`python scripts\n3_historical_membership_dataset.py ...`?
- Git?????

### 00:57:53 +08:00 - ?? atr_reclaim Stage N2 universe gate
- ????? / ?? / ???? / TODO / ?? / Git
- ????? `scripts/n2_universe_audit.py`?? fixed current master ?? listing date??? Binance public-data monthly 1d files ?? historical membership??? N2 audit?listing-enriched master?N0 rerun ? gate decision ???
- ???N2 verdict ? `diagnostic_only_historical_membership_gap`?418 ? current master symbols ? `listed_after_window=152`?`listed_inside_window=49`?`full_window_coverage=208`?`partial_window_coverage=9`???? historical USDT symbols ? `413`??? `147` ??? current master?
- ?????????? clean confirmatory validation ????????? N1??????????`atr_reclaim_0_35` ????????? diagnostic?
- ?????? `python -m compileall scripts\n2_universe_audit.py`?`python tests\test_universe.py`?`python scripts\n2_universe_audit.py ... --fetch-listing-dates`?`python main.py research atr-reclaim-n0-readiness-audit ...`?
- Git?????
...
- Git：`d658a7b` (`Record GPT entry review plan`)。
